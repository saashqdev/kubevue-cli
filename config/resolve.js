...
function getConfig(configPath, packagePath) {
    ...
    if (fs.existsSync(configPath))
        ...
        if (packageKubevue)
            ...
            console.error(chalk.bgRed(' ERROR '), `Cannot find kubevue config! This is not a kubevue project.
...
module.exports = function resolve(configPath = 'kubevue.config.js', theme, externalConfig) {
    ...
    if (!config.globalCSSPath) {
        ...
        if (!fs.existsSync(config.globalCSSPath))
            config.globalCSSPath = path.resolve(require.resolve('@kubevue/doc-loader'), '../node_modules/proto-ui.kubevue/src/base/global.css');
    }
    if (!config.baseCSSPath) {
        ...
        if (!fs.existsSync(config.baseCSSPath))
            config.baseCSSPath = path.resolve(require.resolve('@kubevue/doc-loader'), '../node_modules/proto-ui.kubevue/src/base/base.css');
    }
