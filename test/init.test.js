...
describe('command: init', () => {
    const cli = path.join(__dirname, '../bin/kubevue-init');
    const originalCwd = process.cwd();
    const mockPath = path.join(__dirname, './mock-kubevue-project');
    ...
    describe('init an app type project', () => {
        ...
        let files;
        const type = 'app';
        const projectName = 'kubevue-app-project';
        const projectPath = path.join(__dirname, './mock-kubevue-project/', projectName);
    ...
    describe('init a library type project', () => {
        ...
        let files;
        const type = 'library';
        const projectName = 'kubevue-library-project';
        ...
        it('should init with expected files', (done) => {
            ...
            expect(config.kubevue.type).to.equal(type);
            expect(result.code).to.equal(0);
            done();
        });
    });
